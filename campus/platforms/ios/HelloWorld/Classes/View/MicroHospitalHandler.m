...
- (void)reloadData {
    ...
    if (count % _numberOfColumns) {
        if (self.frame.size.height > (count/_numberOfColumns + 1)*helgth) {
            _scrollView.contentSize = CGSizeMake(_scrollView.frame.size.width, self.frame.size.height + 1);
        }else {
        _scrollView.contentSize = CGSizeMake(_scrollView.frame.size.width, (count/_numberOfColumns + 1)*helgth);
        }
    } else {
        if (self.frame.size.height > (count/_numberOfColumns)*helgth) {
            _scrollView.contentSize = CGSizeMake(_scrollView.frame.size.width, self.frame.size.height + 1);
        } else {
